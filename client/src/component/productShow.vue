...
watch(
  ...
  () => {
    num.value = NUM;
    productsUpper.value = product.value.slice(0, NUM);
    productsLower.value = product.value.slice(0, NUM);
  }
);
...
onMounted(() => {
  ...
  if (instance) {
    ...
    observer.relativeToViewport({ bottom: 100 }).observe(".target", (res) => {
      console.log(product.value.length, productsLower.value.length, num.value);
      productsLower.value = product.value.slice(0, num.value + NUM);
      productsUpper.value = product.value.slice(0, num.value + NUM);
      num.value += NUM;
    });
  } else {
